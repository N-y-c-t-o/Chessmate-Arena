...
logout.addEventListener("click",function(){
    audio.play();
    window.location.href = "http://localhost/Chess_Ai_Project/homePage.php";
...
function getBestMove(fen,callback) {
  ...
  let engine = new Worker("./stockfish/src/stockfish-nnue-16.js");
  //  let engine = new Worker("./stockfish.js-master/src/stockfish-nnue-16.js");
...
let flag=[];let show = 0;
let comp = [1200,600,300,100,-100,-300,-600,-1200];
function playBestMove(bestMove) {
  ...
  let pointDifference=checkPointDifferenceAndChat(boardSquaresArray);
  for(i=0;i<comp.length;i++){
    if(pointDifference!=0){
      if(pointDifference>comp[i]){show = i+1;break;}
    }
  }
    showtext(show);
}
function showtext(index){
  console.log("pintdiff " +index);
    ...
    // Array to store innerHTML values
    var values = elementIds.map(function(id) {
        return document.getElementById(id);
    });
    box = values[0].parentElement;
    if(!flag.includes(index-1)){
      values[index-1].style.display = 'block';
      box.style.display = 'block';
      setTimeout(() => {
        values[index-1].style.display = 'none';
...
// Function to calculate total points for a color
function calculateTotalPoints(color, boardSquaresArray) {
  return boardSquaresArray
    .filter((square) => square.pieceColor === color && square.pieceType !== 'blank')
    .reduce((total, square) => total + materialValues[square.pieceType], 0);
}

// Function to compare points and trigger a conversation
function checkPointDifferenceAndChat(boardSquaresArray) {
  const userColor = isWhiteTurn ? 'white' : 'black';
  const computerColor = isWhiteTurn ? 'black' : 'white';
  
  const userPoints = calculateTotalPoints(userColor, boardSquaresArray);
  const computerPoints = calculateTotalPoints(computerColor, boardSquaresArray);
  
  const pointDifference = computerPoints - userPoints;
...
function sayGreeting(){
  ...
  greeting.parentElement.style.display='block';
  setTimeout(() => {
    greeting.style.display = 'none';
    greeting.parentElement.style.display = 'none';
  }, 4000);
}
